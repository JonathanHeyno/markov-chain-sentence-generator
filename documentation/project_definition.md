...
Because text can include all sorts of symbols and numbers in addition to alphabets (e.g. ?!{]$, etc.) we need to determine how to handle these. I will probably exclude these in the beginning, though there would be a benefit to having them later so that the program can end sentences (in ".", "?", or "!").
...
The idea is presented below. e.g. the word "cat" is stored into the trie structure, and after that we count the frequencies of all the words that follow it in the text to get the probabilities of the subsequent words.
